...
# The released dataset ships prepared under data/, so "local" is the default:
# "roboflow" needs credentials for the authors' private annotation workspace.
SOURCE = "local"
...
# Roboflow's project-level `updated` does not move when an annotation is edited
# in place — observed on this project, where seven images were relabelled a week
# after version 10 was generated while `updated` stayed pinned to the moment
# that version was cut. Only the per-image `lastSet` records the edit, and it is
# not exposed by the search endpoint, so establishing when the annotations last
# changed costs one detail request per image. These bound that scan.
ANNOTATION_SCAN_WORKERS = 16
ANNOTATION_SCAN_PAGE = 100
# The search endpoint drops rows from a single pagination pass, so it is walked
# repeatedly until the id set stops growing (the same tactic image_metadata.py
# uses), and 5xx/429 responses are retried rather than aborting the export.
ANNOTATION_SCAN_PASSES = 10
ROBOFLOW_RETRIES = 4
...
def roboflow_request(request: urllib.request.Request, path: str) -> dict:
    """Send one Roboflow REST request, retrying the failures worth retrying.

    The API returns an occasional 502 under the thousand-odd calls an annotation
    scan makes, and a single one of those would otherwise abort a whole export.
    Server errors and rate limits are retried with a widening delay; a 4xx is a
    real answer about the request and is raised immediately.
    """
    for attempt in range(ROBOFLOW_RETRIES):
        try:
            with urllib.request.urlopen(request, timeout=60) as response:
                return json.load(response)
        except urllib.error.HTTPError as exc:
            retryable = exc.code >= 500 or exc.code == 429
            if not retryable or attempt == ROBOFLOW_RETRIES - 1:
                detail = exc.read()[:200].decode("utf-8", "replace")
                raise SystemExit(
                    f"Roboflow API error on {path}: HTTP {exc.code} {detail}"
                )
        except urllib.error.URLError as exc:
            if attempt == ROBOFLOW_RETRIES - 1:
                raise SystemExit(f"cannot reach the Roboflow API: {exc.reason}")
        time.sleep(2 ** attempt)
    raise SystemExit(f"Roboflow API error on {path}: retries exhausted")
...
def roboflow_get(path: str, api_key: str) -> dict:
    ...
    return roboflow_request(urllib.request.Request(url), path)


def roboflow_post(path: str, api_key: str, payload: dict) -> dict:
    """POST one Roboflow REST endpoint and return the decoded JSON."""
    url = f"{ROBOFLOW_API_URL}/{path}?" + urllib.parse.urlencode({"api_key": api_key})
    ...
    return roboflow_request(request, path)


def project_image_names(config: dict[str, str], api_key: str) -> dict[str, str]:
    """Every image in the project as {id: filename}.

    A single pagination pass is not guaranteed to enumerate the project, so the
    walk is repeated until the id set stops growing.
    """
    path = f"{config['workspace']}/{config['project']}/search"
    by_id: dict[str, str] = {}
    unchanged = 0
    for _ in range(ANNOTATION_SCAN_PASSES):
        before = len(by_id)
        offset = 0
        while True:
            page = roboflow_post(
                path, api_key,
                {"fields": ["id", "name"], "limit": ANNOTATION_SCAN_PAGE,
                 "offset": offset},
            )
            ...
            offset += len(rows)
            if len(rows) < ANNOTATION_SCAN_PAGE or offset >= int(page.get("total", 0)):
                break
...
def prepared_image_names() -> set[str]:
    """Filenames of the images in the dataset as it was last prepared."""
    return {
        image.name
        for split in SPLITS
...
def last_annotation_change(
    config: dict[str, str], api_key: str, newer_than: float = 0.0
) -> tuple[float, str | None]:
    """Return (epoch, filename) of the most recent annotation edit in the dataset.

    Each image carries a ``lastSet`` stamp that moves whenever its annotation is
    written, which is the only signal Roboflow exposes for an edit that leaves
    the image count untouched. It is not a searchable field, so reading it costs
    one request per image.

    The scan covers the images the dataset was last built from, not the whole
    project: this project holds roughly twice as many images as any version
    does, the surplus sitting in the annotation queue outside the dataset, and
    editing one of those says nothing about whether the released split is stale.
    Nothing distinguishes the two groups in the project metadata, so membership
    comes from the prepared split on disk. With no prepared dataset there is no
    previous dataset to be stale against, and the scan is skipped.

    ``newer_than`` short-circuits the walk: the caller only needs to know whether
    *some* edit lands after a version was generated, so the first one that does
    ends it. Returns ``(0.0, None)`` when nothing could be established, which the
    caller reads as "no evidence of change" rather than as evidence of none.
    """
    scope = prepared_image_names()
    if not scope:
        return 0.0, None

    catalogue = project_image_names(config, api_key)
    image_ids = [iid for iid, name in catalogue.items() if name in scope]
    missing = len(scope) - len(image_ids)
    if missing > 0:
        print(f"note: {missing} prepared image(s) no longer in the Roboflow project")
    if not image_ids:
        return 0.0, None

    path = f"{config['workspace']}/{config['project']}/images"
    newest, newest_name = 0.0, None
    done = False

    def probe(image_id: str) -> tuple[float, str | None]:
        if done:  # another worker already cleared the threshold
            return 0.0, None
        image = roboflow_get(f"{path}/{image_id}", api_key).get("image", {})
        # lastSet is milliseconds; every other timestamp here is seconds.
        return float(image.get("lastSet") or 0) / 1000.0, image.get("name")

    with ThreadPoolExecutor(max_workers=ANNOTATION_SCAN_WORKERS) as pool:
        for stamp, name in pool.map(probe, image_ids):
            if stamp > newest:
                newest, newest_name = stamp, name
            if newer_than and newest > newer_than + STALE_TOLERANCE_SECONDS:
                done = True
                break
...
def fetch_project_state(config: dict[str, str], api_key: str) -> tuple[int, float, dict]:
    ...
    ``updated`` catches structural changes, but not an annotation edited in
    place: it stays pinned to the last version generation while images are
    relabelled. ``last_annotation_change`` covers that gap.
    """
...
def version_staleness(
    ...
    Nothing has to be recorded on our side between runs: every signal comes from
    what Roboflow reports right now. A version is stale when it covers a
    different number of images than the project does, when the project's own
    ``updated`` stamp has moved past it, or when any image was annotated after it
    was generated.

    That last check is the one that matters for a relabel. Roboflow's
    project-level ``updated`` is documented-by-behaviour to move on structural
    changes, but it does *not* move when an existing annotation is edited in
    place, so it silently misses exactly the case this pipeline cares about: a
    mask redrawn, a duplicate removed, a degenerate polygon fixed. ``annotated``
    is the newest per-image ``lastSet`` and does catch those; pass 0.0 to skip
    the check when the scan was not run.
    """
    ...
    if updated > created + STALE_TOLERANCE_SECONDS:
        return (f"was generated {project_stamp(created)} but the project was "
                ...
                f"changed since)")
    if annotated > created + STALE_TOLERANCE_SECONDS:
        where = f" ({annotated_name})" if annotated_name else ""
        return (f"was generated {project_stamp(created)} but an annotation was "
                f"edited {project_stamp(annotated)}{where}")
    return None
...
def resolve_roboflow_version(
    ...
    created = float(newest.get("created") or 0.0)
    print(f"Roboflow project: {images} images, modified {project_stamp(updated)} | "
          f"newest version {latest}: {newest.get('images')} images, "
          f"generated {project_stamp(created)}")

    print("checking when the annotations last changed...", flush=True)
    annotated, annotated_name = last_annotation_change(config, api_key, created)
    if annotated:
        print(f"newest annotation edit: {project_stamp(annotated)}"
              + (f" ({annotated_name})" if annotated_name else ""))

    reason = version_staleness(newest, images, updated, annotated, annotated_name)
    if reason is None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "new images and for annotations edited since the newest version, "
            "generates a version when either has happened, and downloads it "
            f"(default: {SOURCE})."
        ),
    )
    parser.add_argument(
