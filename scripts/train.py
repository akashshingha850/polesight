"""PoleSight instance-segmentation training configured by a sweep YAML.

Usage:
    python scripts/train.py                      # every family in the config
    python scripts/train.py yolov8               # one configured family
    python scripts/train.py --eval               # evaluate existing checkpoints
    python scripts/train.py --config my.yaml     # a different sweep definition

The default config is ``configs/train_default.yaml``, the stock-defaults sweep
that produced the fifteen published baselines.
...
try:
    ...
    # Experiment tracking is a convenience, not a requirement: everything the
    # published results rest on is written to disk by _write_json_report. When
    # wandb is absent, or WANDB_DISABLED is set, every wandb call below becomes
    # a no-op so a fresh clone reproduces the sweep without an account.
    wandb = None

if wandb is not None and os.environ.get("WANDB_DISABLED", "").lower() in {"1", "true"}:
    wandb = None


class _NullRun:
    """Context manager standing in for wandb.init() when tracking is off."""
...
def _wandb_init(**kwargs):
    return wandb.init(**kwargs) if wandb is not None else _NullRun()
...
# Scripts live in scripts/; every configured path is resolved from the
# repository root one level up, so a clone works wherever it is checked out.
ROOT_DIR = Path(__file__).resolve().parents[1]
CONFIG_PATH = ROOT_DIR / "configs" / "train_default.yaml"

# Additional segmentation-capable families can be enabled in the config later.
                ...
                for kind in ("box", "mask"):
                    for metric, value in head_results.get(kind, {}).items():
                        if isinstance(value, (int, float)):
                            _wandb_run().summary[f"{split}/{head}/{kind}/{metric}"] = value
    return report
