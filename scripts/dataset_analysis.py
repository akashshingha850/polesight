...
# Every figure — the exploratory dump and the print-size manuscript figures
# (``paper_*.png``) — is written here as PNG, so there is one folder to pick
# figures from. ``paper`` is a symlink to the manuscript repository, which is
# where the figures belong when it is present; a clone without it gets a local
# figures/ instead, so the analysis still runs. POLESIGHT_FIGURE_DIR overrides
# both.
_PAPER_FIGURE_DIR = ROOT_DIR / "paper" / "figures" / "dataset"
FIGURE_DIR = Path(
    os.environ.get("POLESIGHT_FIGURE_DIR")
    or (_PAPER_FIGURE_DIR if (ROOT_DIR / "paper").exists() else ROOT_DIR / "figures")
)
...
# COCO's native-image area cutoffs. We use half-open bins so every annotation
# belongs to exactly one descriptive category: [0, 32^2), [32^2, 96^2), and
# [96^2, infinity), measured in square pixels at the annotation resolution.
SIZE_CATEGORY_THRESHOLDS_PX = (32 ** 2, 96 ** 2)
...
def size_category(area_px):
    small, medium = SIZE_CATEGORY_THRESHOLDS_PX
    if area_px < small:
        return "small"
    if area_px < medium:
        return "medium"
...
def build_report():
    ...
    for split in SPLITS:
        for inst in instances_by_split[split]:
            ...
            mask_area_px = inst.mask_area * img_w * img_h
            raw["bbox_areas_px"].append(bbox_area_px)
            raw["mask_areas_px"].append(mask_area_px)
            ...
            area_px = mask_area_px if inst.kind == "polygon" else bbox_area_px
            raw["size_categories"].append(size_category(area_px))
    ...
    segmentation = {
        ...
        "size_category_thresholds": {
            "small": f"< {SIZE_CATEGORY_THRESHOLDS_PX[0]} px² annotation area",
            "medium": (f"{SIZE_CATEGORY_THRESHOLDS_PX[0]}–"
                       f"<{SIZE_CATEGORY_THRESHOLDS_PX[1]} px² annotation area"),
            "large": f">= {SIZE_CATEGORY_THRESHOLDS_PX[1]} px² annotation area",
        },
        "per_class": {
            class_name(cid): {
...
def generate_figures(report, raw):
    ...
    # 14. Instance size categories per class (COCO native-pixel area cutoffs)
    ...
    ax.legend(frameon=False, ncol=3, loc="upper center", bbox_to_anchor=(0.5, -0.19),
              title="Size (annotation area, px$^2$)")
...
def paper_fig_mask_area_ecdf(report, raw, plt):
    ...
    small_t, medium_t = SIZE_CATEGORY_THRESHOLDS_PX
